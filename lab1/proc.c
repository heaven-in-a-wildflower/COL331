...
struct history_entry history[MAX_HISTORY]; // Allocate memory for history
int history_count = 0;

void add_to_history(const struct proc *p)
{
...
int gethistory(void)
{
  // Sorting history[] by PID using bubble sort
  for (int i = 0; i < history_count - 1; i++)
  {
    for (int j = 0; j < history_count - i - 1; j++)
    {
      ...
      {
        // Swap history[j] and history[j + 1]
  ...
  // Print the sorted history entries
...
int fork(void)
{
  ...
  *np->tf = *curproc->tf;
  ...
  // ✅ Inherit the NUM_SYSCALLS-bit array from parent
  for (i = 0; i < NUM_SYSCALLS; i++)
  {
    // np->syscall_bitmask[i] = curproc->syscall_bitmask[i];
    np->syscall_bitmask[i] = 0;
  }

  //Intermed sh by default
  np->real_sh=1;
